...
- `speech`: 라인이 완성되는 즉시 TTS queue로 보낸다. 사용자에게 들려줄 진행상황, 조사 내용, 판단, 짧은 요약은 `speech`를 사용한다.
- `command`: terminal/UI log에 표시하고 기본적으로 말하지 않는다. shell command, path, URL, flag, stack trace, raw log, 실행 목록처럼 TTS로 듣기 어려운 요소만 담는다.
- `status`: silent UI state에만 사용하고 기본적으로 말하지 않는다.
- `error`: 표시하고 짧게 말한다.
...
{"op":"voice-agent-ui","type":"state","state":"wake_rejected","text":"wake phrase not matched"}
...
- `wake_rejected`는 STT 결과가 나왔지만 configured wake phrase와 맞지 않은 상태다.
- `submitting`은 STT 결과를 Transcript로 확정한 뒤 Codex/Claude에 prompt를 넘기는 짧은 상태다.
- `speech` event text는 audio circle 근처에 다중 줄로 표시한다.
