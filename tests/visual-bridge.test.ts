...
test("Qt companion is native QML and avoids browser/webview imports", async () => {
  ...
  assert.match(qml, /root\.pushChat\("assistant", "answer", event\.text\)/u);
  assert.match(qml, /root\.pushChat\("assistant", "command", event\.text\)/u);
  assert.match(qml, /if \(!event\.transient\) root\.pushChat\("assistant", "status", root\.statusText\)/u);
  assert.match(qml, /root\.pushChat\("assistant", "approval", event\.text\)/u);
  assert.match(qml, /root\.pushChat\("assistant", "popup"/u);
  assert.match(qml, /root\.replaceChatHistory\(event\.entries \|\| \[\]\)/u);
  assert.match(qml, /root\.sendControl\("clear_chat_history"\)/u);
  assert.match(qml, /next\.length - 100/u);
  assert.match(qml, /shouldSkipChatItem/u);
...
test("macOS native companion is AppKit and avoids browser/webview imports", async () => {
  ...
  assert.match(swift, /items\.removeFirst\(items\.count - 100\)/u);
  ...
  assert.match(swift, /pushChat\(role: "assistant", kind: "answer"/u);
  assert.match(swift, /pushChat\(role: "assistant", kind: "command"/u);
  assert.match(swift, /event\["transient"\] as\? Bool != true/u);
  assert.match(swift, /pushChat\(role: "assistant", kind: "status"/u);
  assert.match(swift, /pushChat\(role: "assistant", kind: "approval"/u);
  assert.match(swift, /pushChat\(role: "assistant", kind: "popup"/u);
  assert.match(swift, /sendControl\("clear_chat_history"\)/u);
  assert.match(swift, /shouldSkipItem\(kind:/u);
  ...
  assert.match(swift, /private func markdownTableHtml/u);
  assert.match(swift, /<div class="table-wrap"><table>/u);
  assert.match(swift, /private func markdownListHtml/u);
  assert.match(swift, /task-list/u);
  assert.match(swift, /private func markdownBlockquoteHtml/u);
  assert.match(swift, /private func markdownInlineLink/u);
  assert.match(swift, /private func markdownAutoLink/u);
  assert.match(swift, /<del>/u);
  assert.match(swift, /private final class MarkdownRenderContext/u);
  assert.match(swift, /private func extractMarkdownFootnotes/u);
  assert.match(swift, /private func markdownFootnoteReference/u);
  assert.match(swift, /class=\\?"footnotes\\?"/u);
  assert.match(swift, /private func markdownDetailsHtml/u);
  assert.match(swift, /<details><summary>/u);
